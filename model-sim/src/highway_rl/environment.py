...
class HighwayEnvironment:
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        if "speed" in info:
            self.episode_metrics["distance_traveled"] += (
                info["speed"] * 0.1
            )  # dt = 0.1s
    ...
    def get_optimized_config(cls, scenario: str) -> Dict:
        ...
        if scenario == "highway":
            ...
            base_config.update(
                {"collision_reward": -1, "normalize_reward": True, "lap_reward": 1}
            )
    ...
    def __init__(self, scenarios: List[str], render_mode: Optional[str] = None):
        ...
        for scenario in scenarios:
            ...
            self.environments[scenario] = HighwayEnvironment(
                scenario=scenario, render_mode=render_mode, config_overrides=config
            )

        self.current_env = self.environments[scenarios[0]]

    def switch_scenario(self, scenario: Optional[str] = None) -> bool:

// @ts-nocheck — vendored from standardagents/jevpilot src/vehicle-model.js
import * as THREE from "three";
import { RoundedBoxGeometry } from "three/addons/geometries/RoundedBoxGeometry.js";
import { mergeGeometries } from "three/addons/utils/BufferGeometryUtils.js";
import { physical, material } from "./materials";

export interface DetailedCarOptions {
  motorcycle?: boolean;
  /** RoundedBox segment count — 4 reads smoother at chase-cam distance (JevPilot portfolio bar). */
  segments?: number;
  /** Merge draw calls by material; disable for debug silhouette inspection. */
  merge?: boolean;
}

/**
 * JevPilot procedural sedan — RoundedBox + glass quads, clearcoat/glass/rubber/chrome/emissive.
 * Traffic fallback when GLB unavailable. Ghost in planning = kinematic clone, NOT mesh opacity.
 */
export function detailedCar(
  color = "#d6d9df",
  motorcycleOrOptions: boolean | DetailedCarOptions = false,
) {
  const options: DetailedCarOptions =
    typeof motorcycleOrOptions === "boolean"
      ? { motorcycle: motorcycleOrOptions }
      : motorcycleOrOptions;
  const motorcycle = options.motorcycle ?? false;
  ...
  const box = (w: number, h: number, d: number, x: number, y: number, z: number, mat: THREE.Material, radius = 0.025) =>
    mesh(new RoundedBoxGeometry(w, h, d, segments, radius), mat, x, y, z);
  const quad = (points: number[][], mat: THREE.Material) => {
  ...
  if (motorcycle) {
    ...
    for (const side of [-1, 1]) {
      ...
      for (const z of [-0.2, 0.88]) box(0.05, 0.035, 0.22, side * 0.95, 0.92, z, chrome);
  ...
  for (const z of motorcycle ? [-0.77, 0.77] : [-1.29, 1.28]) {
    for (const side of motorcycle ? [0] : [-1, 1]) {
      const x = side * 0.87;
      const tire = mesh(
        new THREE.TorusGeometry(motorcycle ? 0.28 : 0.285, motorcycle ? 0.085 : 0.1, 10, 24),
        rubber,
        x,
        0.39,
        z,
  ...
  group.userData.width = motorcycle ? 0.8 : 1.9;
  group.userData.depth = motorcycle ? 2.3 : 4.16;
  return group;
}

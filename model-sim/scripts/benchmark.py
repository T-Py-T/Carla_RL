    ...
    for height, width, channels, batch_size in image_configs:
        try:
            with tf.device(device):
                ...
                print(
                    f"  Batch {batch_size} ({height}x{width}): {elapsed:.3f}s/batch ({fps:.1f} FPS)"
                )
...
def benchmark_rl_specific():
    ...
    try:
        ...
        with tf.device(device):
            ...
            model = models.model_head_hidden_dense(
                *model_base, outputs=len(settings.ACTIONS)
            )
            ...
            for batch_size in batch_sizes:
                try:
                    ...
                    fps = batch_size / elapsed
                    print(
                        f"  RL Model Batch {batch_size}: {elapsed:.4f}s/batch ({fps:.1f} FPS)"
                    )
    ...
    try:
        with tf.device(device):
            ...
            for i in range(1, 6):
                ...
                print(
                    f"  Tensor {size}x{size}: {memory_current.used / (1024**3):.1f} GB used"
                )

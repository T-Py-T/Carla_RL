...
if __name__ == "__main__":
    ...
    p = Process(
        target=check_weights_size,
        args=(hparams["model_path"] if hparams else False, weights_size),
        daemon=True,
    )
    p.start()
    ...
    update_target_every = Value(
        "L", hparams["update_target_every"] if hparams else settings.UPDATE_TARGET_EVERY
    )
    ...
    for agent in range(settings.AGENTS):
        if hparams:
            ...
            agent_show_preview.append(
                Array("f", [(agent + 1) in settings.AGENT_SHOW_PREVIEW, 0, 0, 0, 0, 0])
            )
    save_checkpoint_every = Value(
        "L",
        hparams["save_checkpoint_every"] if hparams else settings.SAVE_CHECKPOINT_EVERY,
    )
    seconds_per_episode = Value(
        "L", hparams["seconds_per_episode"] if hparams else settings.SECONDS_PER_EPISODE
    )
    ...
    episode_stats = Array(
        ...
        + [-(10**6) for _ in range((CarlaEnv.action_space_size + 1) * 3)],
    )
    stop = Value("B", 0)
    agent_stats = []
    ...
    for agent in range(settings.AGENTS):
        carla_instance = (
            1
            if not len(settings.AGENT_CARLA_INSTANCE)
            or settings.AGENT_CARLA_INSTANCE[agent] > settings.CARLA_HOSTS_NO
            else settings.AGENT_CARLA_INSTANCE[agent]
        )
    ...
    for process_no in range(settings.CARLA_HOSTS_NO):
        ...
        carla_settings = CarlaEnvSettings(
            process_no,
            agents_in_carla_instance[process_no],
            stop,
            car_npcs,
            carla_settings_process_stats,
        )
        carla_settings_thread = Thread(
            target=carla_settings.update_settings_in_loop, daemon=True
        )
    ...
    trainer_process = Process(
        target=run_trainer,
        args=(
            hparams["model_path"] if hparams else False,
            hparams["logdir"] if hparams else False,
            stop,
            weights,
            weights_iteration,
            episode,
            ...
            episode_stats,
            optimizer,
            hparams["models"] if hparams else [],
            car_npcs,
            carla_settings_stats,
            carla_fps,
        ),
    ...
    for agent in range(settings.AGENTS):
        ...
        p = Process(
            ...
            args=(
                ...
                episode,
                epsilon,
                agent_show_preview[agent],
                weights,
                weights_iteration,
                transitions,
                tensorboard_stats,
                agent_stats[agent],
                carla_frametimes_list[carla_instance - 1],
                seconds_per_episode,
            ),
            daemon=True,
        )
    ...
    while True:
        ...
        if stop.value in [
            STOP.running,
            STOP.carla_simulator_error,
            STOP.restarting_carla_simulator,
            STOP.carla_simulator_restarted,
        ]:
        ...
        if stop.value != STOP.carla_simulator_error:
            for process_no in range(settings.CARLA_HOSTS_NO):
                for _ in range(carla_frametimes_list[process_no].qsize()):
                    try:
                        carla_fps_counters[process_no].append(
                            carla_frametimes_list[process_no].get(True, 0.1)
                        )
                    except Empty:
                        break
                carla_fps[process_no].value = (
                    len(carla_fps_counters[process_no])
                    / sum(carla_fps_counters[process_no])
                    if sum(carla_fps_counters[process_no]) > 0
                    else 0
                )
        ...
        if (
            stop.value == STOP.carla_simulator_error
            and settings.CARLA_HOSTS_TYPE == "local"
        ):
            ...
            if time.time() > carla_check + 15:
                ...
                for process_no in range(settings.CARLA_HOSTS_NO):
                    while (
                        carla_settings_threads[process_no][1].state
                        != CARLA_SETTINGS_STATE.restarting
                    ):
                        time.sleep(0.1)
                restart_carla()
                for process_no in range(settings.CARLA_HOSTS_NO):

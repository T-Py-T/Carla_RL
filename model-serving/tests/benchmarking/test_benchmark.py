...
from src.benchmarking.benchmark import (
    BenchmarkEngine, BenchmarkConfig, LatencyStats, ThroughputStats, MemoryStats
)
...
class TestBenchmarkEngine:
    ...
    def test_generate_report(self):
        ...
        result.latency_stats = LatencyStats(
            p50_ms=5.0, p95_ms=10.0, p99_ms=20.0,
            mean_ms=6.0, std_ms=2.0, min_ms=1.0, max_ms=25.0, total_measurements=100
        )
        result.throughput_stats = ThroughputStats(
            requests_per_second=1500.0, total_requests=1000, total_duration_s=10.0,
            successful_requests=950, failed_requests=50, error_rate=0.05
        )
        result.memory_stats = MemoryStats(
            peak_memory_mb=512.0, average_memory_mb=400.0,
            memory_growth_mb=100.0, memory_efficiency=10.0
        )

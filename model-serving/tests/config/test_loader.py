...
import pytest
import yaml
from src.config.loader import ConfigLoader, create_config_loader, load_config
...
class TestConfigLoader:
    ...
    def test_load_config_hierarchical_override(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.dict(os.environ, {"APP_SERVER_PORT": "8080", "APP_DEBUG": "false"}):
                loader.add_env_source("APP_")
                config = loader.load_config()
...
class TestLoadConfigFunction:
    ...
    def test_load_config_with_file(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            config_file = Path(temp_dir) / "test.yaml"
            config_data = {"environment": "staging", "server": {"port": 8080}}
            with open(config_file, "w") as f:
                yaml.dump(config_data, f)
    ...
    def test_load_config_with_env_prefix(self):
        ...
        with patch.dict(
            os.environ, {"TEST_ENVIRONMENT": "development", "TEST_SERVER_PORT": "9000"}
        ):
    ...
    def test_create_config_loader(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            loader = create_config_loader(
                config_dir=Path(temp_dir), env_prefix="TEST_", config_files=config_files
            )

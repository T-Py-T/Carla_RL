...
def setup_artifact_manager(
    artifacts_dir: str, content_storage_dir: Optional[str] = None
...
def check_content_exists(
    storage: ContentAddressableStorage, content_hash: str, json_output: bool = False
) -> None:
    """Check if content exists in storage."""
    exists = storage.content_exists(content_hash)

    if json_output:
        output = {"content_hash": content_hash, "exists": exists}
        print(json.dumps(output, indent=2))
    else:
        if exists:
...
def list_content(
    ...
    if json_output:
        ...
        for ref in content_list:
            print(
                f"{ref.content_hash}  {ref.size:>10,} bytes  {ref.created_at}  (accessed {ref.access_count} times)"
            )
...
def delete_content(
    ...
    try:
        ...
        if json_output:
            output = {"content_hash": content_hash, "deleted": success}
            print(json.dumps(output, indent=2))
        else:
            if success:
...
def show_storage_stats(storage: ContentAddressableStorage, json_output: bool = False) -> None:
...
def cleanup_orphaned(storage: ContentAddressableStorage, json_output: bool = False) -> None:
    """Clean up orphaned content."""
    try:
        orphaned_count = storage.cleanup_orphaned_content()

        if json_output:
            output = {"orphaned_files_cleaned": orphaned_count}
            print(json.dumps(output, indent=2))
        else:
...
def verify_integrity(storage: ContentAddressableStorage, json_output: bool = False) -> None:
...
Examples:
  ...
  python content_storage_manager.py --storage-dir /path/to/storage verify
    ...
    parser.add_argument("--storage-dir", required=True, help="Path to content storage directory")
    ...
    store_parser.add_argument("--content", required=True, help="Path to content file")
    ...
    retrieve_parser.add_argument("--hash", required=True, help=CONTENT_HASH_HELP)
    retrieve_parser.add_argument(
        "--output", help="Output file path (if not provided, prints to stdout)"
    )

    # Check command
    check_parser = subparsers.add_parser("check", help="Check if content exists")
    check_parser.add_argument("--hash", required=True, help=CONTENT_HASH_HELP)

    # Info command
    info_parser = subparsers.add_parser("info", help="Get content information")
    info_parser.add_argument("--hash", required=True, help=CONTENT_HASH_HELP)

    # List command
    list_parser = subparsers.add_parser("list", help="List all content")
    list_parser.add_argument("--limit", type=int, help="Limit number of items")

    # Delete command
    delete_parser = subparsers.add_parser("delete", help="Delete content")
    delete_parser.add_argument("--hash", required=True, help=CONTENT_HASH_HELP)

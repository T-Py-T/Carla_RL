...
def load_performance_thresholds(
    config_file: str = "config/performance-thresholds.yaml",
) -> Dict[str, Any]:
...
def run_environment_aware_performance_test(
    ...
    print(
        f"Thresholds: P50<{env_thresholds['p50_latency_ms']}ms, "
    ...
    test_result = {
        ...
        "performance_analysis": {
            "throughput_efficiency": (
                result.throughput_stats.requests_per_second / env_thresholds["throughput_rps"]
            )
            * 100,
            "latency_efficiency": (env_thresholds["p50_latency_ms"] / result.latency_stats.p50_ms)
            * 100,
            "memory_efficiency": (
                env_thresholds["memory_usage_mb"] / result.memory_stats.peak_memory_mb
            )
            * 100,
        },
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run environment-aware performance tests")
    ...
    parser.add_argument(
        "--output", "-o", type=str, help="Output file for test results (JSON format)"
    )

...
def run_performance_regression_test(
    ...
    if baseline:
        ...
        p50_diff = calculate_percentage_diff(
            result.latency_stats.p50_ms, baseline_metrics.get("p50_latency_ms", 0)
        )
        ...
        significant_regression = (
            p50_diff > threshold_percent
            or p95_diff > threshold_percent
            or throughput_diff < -threshold_percent
        )
...
def main():
    ...
    try:
        result = run_performance_regression_test(
            config, args.baseline_file, args.regression_threshold
        )

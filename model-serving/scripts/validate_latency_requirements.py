...
def validate_latency_requirements(
    config: BenchmarkConfig, target_p50_ms: float = 10.0, iterations: int = 1000
) -> Dict[str, Any]:
    ...
    validation_result = {
        ...
        "p50_difference_percent": ((result.latency_stats.p50_ms - target_p50_ms) / target_p50_ms)
        * 100.0,
...
def generate_latency_recommendations(latency_stats, target_p50_ms: float) -> list:
    ...
    if latency_stats.p50_ms > target_p50_ms:
        recommendations.append(
            f"P50 latency ({latency_stats.p50_ms:.2f}ms) exceeds target ({target_p50_ms}ms)"
        )
    ...
    if latency_stats.coefficient_of_variation > 0.3:
        recommendations.append(
            "High latency variance detected - investigate performance consistency"
        )
    ...
    if latency_stats.outlier_percentage > 5:
        recommendations.append(
            f"High outlier rate ({latency_stats.outlier_percentage:.1f}%) - investigate causes"
        )
    ...
    if latency_stats.skewness > 1.0:
        recommendations.append(
            "Right-skewed distribution - investigate occasional high-latency requests"
        )
    ...
    if latency_stats.p95_ms > target_p50_ms * 2:
        recommendations.append(
            "P95 latency significantly higher than target - optimize worst-case performance"
        )
...
def main():
    ...
    # Run validation
    try:
        result = validate_latency_requirements(config, args.target_p50, args.iterations)
        ...
        print(
            f"Difference: {result['p50_difference_ms']:+.3f}ms ({result['p50_difference_percent']:+.1f}%)"
        )
        ...
        print(
            f"  Outliers: {dist['distribution_shape']['outlier_count']} ({dist['distribution_shape']['outlier_percentage']:.1f}%)"
        )

...
class CPUOptimizer:
    ...
    def _enable_avx_optimizations(self) -> Dict[str, Any]:
        ...
        setattr(torch.backends.mkldnn, "enabled", True)
        setattr(torch.backends.mkldnn, "allow_tf32", True)
    ...
    def _enable_sse_optimizations(self) -> Dict[str, Any]:
        ...
        setattr(torch.backends.mkl, "enabled", True)
    ...
    def _get_optimal_thread_count(self, hardware_info: HardwareInfo) -> int:
        ...
        if self.config.max_threads:
            return int(min(self.config.max_threads, hardware_info.cpu.threads))

        # Use logical cores but cap at 8 for inference
        return int(min(hardware_info.cpu.threads, 8))
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        metrics: Dict[str, Any] = {
            ...
            "mkl_enabled": getattr(torch.backends.mkl, "enabled", False),

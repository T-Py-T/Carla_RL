...
class DiffResult:
    ...
    def __post_init__(self) -> None:
        ...
        self.has_changes = (
            self.summary["added"] + self.summary["removed"] + self.summary["changed"] > 0
        )
...
class ConfigDiff:
    ...
    def compare_configs(
        ...
        return DiffResult(has_changes=len(items) > 0, total_changes=len(items), items=items)

    def _convert_deepdiff_to_items(
        self, diff: DeepDiff, old_dict: Dict[str, Any], new_dict: Dict[str, Any]
    ) -> List[DiffItem]:
    ...
    def add_custom_comparator(self, path: str, comparator: Callable[..., Any]) -> None:
        """
...
def compare_config_files(
    file1: Union[str, Path], file2: Union[str, Path], format: str = "auto"
) -> DiffResult:
    """
...
def _load_config_from_file(
    file_path: Union[str, Path], file_format: str = "auto"
) -> Dict[str, Any]:
...
def _format_diff_item(item: DiffItem) -> List[str]:
    ...
    severity_icon = {"error": "❌", "warning": "⚠️", "info": "ℹ️"}.get(item.severity, "ℹ️")
    lines = [
...
def _apply_deployment_rule(
    rule_name: str, rule_config: Dict[str, Any], diff_result: DiffResult
) -> bool:

...
class ConfigValidator:
    ...
    @staticmethod
    def _pydantic_issues(config: BaseConfig, config_data: Dict[str, Any]) -> List[ValidationIssue]:
    ...
    def _field_validation_issues(
        ...
        for field_name, validators in self._validators.items():
            ...
            for validator_func in validators:
                try:
                    self._append_result(issues, validator_func(field_value, config))
...
def validate_host(value: Any, config: BaseConfig) -> Optional[ValidationIssue]:
    ...
    # Basic IP/hostname validation. This compares configuration strings; it
    # does not bind a socket.
    if value not in [  # nosec B104
        "0.0.0.0",
        "127.0.0.1",
        "localhost",
    ]:
...
def validate_database_config(config: AppConfig) -> List[ValidationIssue]:
    ...
    # Handle both enum objects and strings
    backend = (
        db_config.backend.value if hasattr(db_config.backend, "value") else str(db_config.backend)
    )
...
def validate_cache_config(config: AppConfig) -> List[ValidationIssue]:
    ...
    # Handle both enum objects and strings
    backend = (
        cache_config.backend.value
        if hasattr(cache_config.backend, "value")
        else str(cache_config.backend)
    )
...
def validate_config(
    config: BaseConfig, validator: Optional[ConfigValidator] = None
) -> ValidationResult:
    """

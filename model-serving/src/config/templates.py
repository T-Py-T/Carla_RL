...
class TemplateEngine:
    ...
    def __init__(self, templates_dir: Optional[Path] = None):
        ...
        # These templates emit configuration files, not HTML; escaping would
        # corrupt quoted YAML/TOML/JSON values.
...
""",
            variables={
                # This is template data, not a socket operation.
...
cache:
  backend: "memory"
""",
            variables={"port": 8001, "model_backend": "pytorch"},
        )
    ...
    def load_template_from_file(self, file_path: Union[str, Path]) -> ConfigTemplate:
        ...
        template = ConfigTemplate(
            name=file_path.stem, template_content=content, variables=variables
        )
    ...
    def generate_config(
        ...
        context.update({"environment": environment.value, "env": environment.value})

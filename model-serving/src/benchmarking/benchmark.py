...
class BenchmarkEngine:
    ...
    def _validate_latency_statistics(self, latencies: np.ndarray, mean: float, std: float) -> None:
        ...
        if np.any(latencies > 10000):  # 10 seconds
            print(
                "Warning: Extremely high latency measurements detected - check for performance issues"
            )
        ...
        outliers = latencies[
            (latencies < q1 - outlier_threshold) | (latencies > q3 + outlier_threshold)
        ]

        if len(outliers) > len(latencies) * 0.05:  # More than 5% outliers
            print(
                f"Warning: {len(outliers)} outliers detected in latency measurements - consider increasing iterations"
            )
    ...
    async def run_benchmark(self, inference_func: Callable, batch_size: int = 1) -> BenchmarkResult:
        ...
        throughput_requirement_met = (
            throughput_stats.requests_per_second >= self.config.throughput_threshold_rps
        )
    ...
    def run_batch_size_optimization(self, inference_func: Callable) -> List[BenchmarkResult]:
    ...
    def _calculate_batch_size_score(self, result: BenchmarkResult, batch_size: int) -> float:
        ...
        if result.memory_stats.peak_memory_mb > 0:
            efficiency_score = (
                result.throughput_stats.requests_per_second / result.memory_stats.peak_memory_mb
            )
    ...
    def run_dynamic_batch_size_optimization(
        ...
        for batch_size in range(1, 65):
            ...
            try:
                ...
                # Check constraints
                if (
                    result.latency_stats.p50_ms > target_latency_ms
                    or result.memory_stats.peak_memory_mb > max_memory_mb
                ):
    ...
    def collect_hardware_baseline(
        self, inference_func: Callable, save_baseline: bool = True
    ) -> Dict[str, Any]:
    ...
    def compare_with_baseline(
        self, current_result: BenchmarkResult, baseline: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:

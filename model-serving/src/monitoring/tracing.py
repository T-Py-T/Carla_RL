...
class TracingMiddleware:
    ...
    def __init__(self, service_name: str = "carla-rl-serving"):
        ...
        self.current_trace_id: ContextVar[Optional[str]] = ContextVar(
            "current_trace_id", default=None
        )
        self.current_span_id: ContextVar[Optional[str]] = ContextVar(
    ...
    def clear_trace(self, trace_id: str):
        ...
        with self._lock:
            spans_to_remove = [
                span_id for span_id, span in self.spans.items() if span.trace_id == trace_id
            ]
            ...
            "summary": {
                ...
                "operations": {
                    op_name: {
                        ...
                        "avg_duration_ms": sum(span.duration_ms or 0 for span in op_spans)
                        / len(op_spans),
                        "error_count": sum(
                            1 for span in op_spans if span.status == SpanStatus.ERROR
                        ),
                    }

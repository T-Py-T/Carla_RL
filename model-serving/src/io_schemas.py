...
class Observation(BaseModel):
    ...
    speed: float = Field(..., ge=0.0, le=200.0, description="Vehicle speed in km/h")
...
class Action(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"throttle": 0.7, "brake": 0.0, "steer": 0.1}}
    )
...
class PredictResponse(BaseModel):
    ...
    version: str = Field(..., description="Model version used for inference")
    timingMs: float = Field(..., ge=0.0, description="Inference time in milliseconds")
    deterministic: bool = Field(..., description="Whether deterministic inference was used")
...
class HealthResponse(BaseModel):
    ...
    git: str = Field(..., description="Git commit SHA")
...
class MetadataResponse(BaseModel):
    ...
    modelName: str = Field(..., description="Name of the loaded model")
    version: str = Field(..., description="Model version")
    device: str = Field(..., description="Inference device")
    inputShape: list[int] = Field(..., description="Expected input tensor shape")
...
class WarmupResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"status": "warmed", "timingMs": 150.2, "device": "cpu"}}
    )

    status: str = Field(..., description="Warmup status")
    timingMs: float = Field(..., ge=0.0, description="Warmup time in milliseconds")
    device: str = Field(..., description="Device used for warmup")
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "details": {"field": "sensors", "value": "too_large"},
                "timestamp": 1695825600.0,
            }
        }
    )
